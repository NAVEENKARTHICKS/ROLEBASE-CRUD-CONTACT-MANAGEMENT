...
app.post('/Signup',(request,response)=>{
   ...
   let sql = 'insert into details(username,password,firstname,lastname,email,mobile,location,descript,role) values (?,?,?,?,?,?,?,?,?)';

   c.query(sql,[email,password,firstname,lastname,email,mobile,location,descript,role],(error,result)=>{
    if(error){
        let s = {"status":"error"}
        response.send(s);
    }
...
app.get('/Employerdashboard',(request,response)=>{
    let sql='select * from details where role="Employer"';
    c.query(sql,(error,result)=>{
        if(error){
            response.send(error);
        }
...
app.put('/updatedata/:id',(request,response)=>{
    ...
    let sql = 'update details set firstname=?,lastname=?,email=?,mobile=?,location=?,descript=?,password=? where id=?';
